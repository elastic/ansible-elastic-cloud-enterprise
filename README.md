...
Supported container-engine hosts include Ubuntu 16.04/20.04/22.04/24.04 (Docker), SLES 15 (Docker), Rocky 8/9 (Podman), RHEL 8 (Docker or Podman), and RHEL 9 (Podman). RHEL 9 with Docker, RHEL 10 with Podman, and RHEL 10 with Docker 29 are experimental in this role (not official support-matrix combinations) and emit a warning at runtime. RHEL 10 is also not listed in Galaxy yet. RHEL 10 Docker uses the CentOS 9 stream repo as a stand-in since no official RHEL 10 Docker CE repo exists.
