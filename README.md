...
- [Supported Docker Versions](https://www.elastic.co/guide/en/cloud-enterprise/2.7/ece-software-prereq.html#ece-linux-docker)
  - `docker_version`: Must be a key in the OS `docker_version_map` (or a bare major that resolves to one). A bare major (`24`, `29`) installs the latest mapped line of that major; `24.0` / `25.0` install that minor line only when the OS ships it. There is no `29.0` key — ECE does not support Docker 29.0–29.2. Each OS map pins `docker-ce` / `docker-ce-cli` / `containerd.io` together (Docker 24/25 stay on containerd 1.7.*; Docker 29 uses 2.x). Last supported version on Centos 7/8 and RHEL 7/8 is 20.10, Ubuntu 16, Ubuntu 18 and SLES 12 is 19.03.
